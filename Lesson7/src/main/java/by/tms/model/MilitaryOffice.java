...
public class MilitaryOffice {
    ...
    public int getRecruitsName(String name) {
        int countRecruit = 0;
        ...
        if (name != null) {
          ...
          /*  for (int i = 0; i < personRegistry.citizens.length; i++) {
            ...
            for (int i = 0; i < personRegistry.getPersonRegistryList().size(); i++) {
                Person person = personRegistry.getPersonRegistryList().get(i);
                if (person.getName().toLowerCase().trim().equals(name.toLowerCase().trim()) && person.getAge() >= MIN_AGE && person.getAge() <= MAX_AGE && person.getGender().toLowerCase().equals(MALE)) {
                    countRecruit++;
                }
            }
    ...
    public int getRecruitsByCity(String city) {
        int countRecruit = 0;
        if (city != null) {
    ...
    public int getRecruitsByAge(int ageMin, int ageMax) {
        int countRecruit = 0;
